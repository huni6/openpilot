...
LIGHT_CLUSTER_THEME = ClusterTheme(
    name="light",
    is_dark=False,
    bg=(255, 255, 255),               # PLeOS 특유의 순백색 배경
    panel_bg=(248, 249, 250),         # 아주 연한 패널 배경
    text=(10, 10, 10),                # 대비를 높여 텍스트를 또렷하게 (순흑색)
    muted=(140, 145, 150),            # 고급스러운 실버 그레이 톤
    faint=(230, 230, 235),            
    road=(255, 255, 255),             # 도로는 배경과 일치시켜 미니멀리즘 극대화
    road_edge=(200, 204, 208),        # 도로 경계선은 아주 연하고 부드럽게
    lane_marking_border=(180, 185, 190, 255),
    road_edge_backing=(255, 255, 255, 0),
    path_shadow=(0, 0, 0, 20),        # 그림자를 확 줄여서 플랫(Flat)한 느낌 강조
    path_uncertainty=(0, 210, 160, 40), 
    path_body=(0, 200, 110, 220),     # 현대 PLeOS 스타일의 네온 그린/민트색 주행 경로
    path_highlight=(180, 255, 220, 240),
    ...
    primary_vehicle=(30, 32, 35),     # 무게감 있는 유광 블랙 (블랙 EV6에 매칭)
    model_vehicle=(220, 225, 230),    # 주변 NPC 차량은 흰색/밝은 회색으로 통일하여 깔끔하게
...
DARK_CLUSTER_THEME = ClusterTheme(
    ...
    road=(12, 14, 18),
    road_edge=(70, 75, 80),
    lane_marking_border=(100, 105, 110, 200),
    road_edge_backing=(0, 0, 0, 0),
    path_shadow=(0, 0, 0, 100),
    path_uncertainty=(0, 180, 140, 50),
    path_body=(0, 200, 110, 220),     # 다크모드에서도 빛나는 네온 그린 포인트
    path_highlight=(180, 255, 220, 240),
    ...
    primary_vehicle=(30, 32, 35),     # 다크모드에서도 내 차는 블랙
    model_vehicle=(160, 165, 170),    # 주변 NPC 차량
