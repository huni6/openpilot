...
FOLLOW_STATUS_CENTER_X = GEAR_STATUS_CENTER_X + 40
FOLLOW_STATUS_W = 160
FOLLOW_STATUS_H = 42.0 * DRIVE_STATUS_SCALE
FOLLOW_STATUS_GAP_BARS = 4
FOLLOW_GAP_ACTIVE = (*BLUE, 255)
...
AMBIENT_BSM_LIGHT_COLOR = (255, 152, 88)
AMBIENT_BSM_DARK_EDGE_ALPHA = 190.0
AMBIENT_BSM_DARK_CORE_ALPHA = 90.0
AMBIENT_BSM_LIGHT_EDGE_ALPHA = 235.0
AMBIENT_BSM_LIGHT_CORE_ALPHA = 125.0
...
EGO_VEHICLE_TEXTURE_SCALE = 1.05
...
def gear_status_color_for_theme(gear_text: str, theme: ClusterTheme) -> tuple[int, int, int]:
    return WHITE if theme.is_dark else BLACK
...
class ClusterUiRenderer:
    ...
    def _draw_radar_point(self, point: RadarPointMarker) -> None:
        ...
        def scale_color(factor: float) -> tuple[int, int, int, int]:
            return (
        ...
        def highlight_color(amount: float) -> tuple[int, int, int, int]:
            return (
                int(clamp(color[0] + (255 - color[0]) * amount, 0, 255)),
                int(clamp(color[1] + (255 - color[1]) * amount, 0, 255)),
                int(clamp(color[2] + (255 - color[2]) * amount, 0, 255)),
                min(235, alpha),
            )

        width_m = max(1.05, point.radius_m * 6.5)
        length_m = max(2.45, width_m * 2.35)
        height_m = max(0.45, width_m * 0.46)
        vehicle = VehicleBox(
            center=Vec3(point.center.x, point.center.y, height_m * 0.5),
            right_x=1.0,
            right_y=0.0,
            forward_x=0.0,
            forward_y=1.0,
            ...
            height_m=height_m,
            body_color=color,
            side_color=scale_color(0.78),
            rear_color=scale_color(0.68),
            top_highlight=highlight_color(0.16),
            outline_color=scale_color(0.52),
            confidence=clamp(point.probability if point.probability is not None else 0.72, 0.0, 1.0),
            label=point.label,
            source="radarPoint",
    ...
    def _draw_ambient_bsm_edges(self, state: ClusterUiState) -> None:
        ...
        if self._current_theme().is_dark:
            color = AMBIENT_BSM_DARK_COLOR
            edge_alpha = AMBIENT_BSM_DARK_EDGE_ALPHA
            core_alpha = AMBIENT_BSM_DARK_CORE_ALPHA
        else:
            color = AMBIENT_BSM_LIGHT_COLOR
            edge_alpha = AMBIENT_BSM_LIGHT_EDGE_ALPHA
            core_alpha = AMBIENT_BSM_LIGHT_CORE_ALPHA
        now = time.monotonic()
        ...
        if self._ambient_bsm_left_opacity > 0.0:
            self._draw_ambient_bsm_edge(
                "left", self._ambient_bsm_left_opacity, color, edge_alpha, core_alpha
            )
        if self._ambient_bsm_right_opacity > 0.0:
            self._draw_ambient_bsm_edge(
                "right", self._ambient_bsm_right_opacity, color, edge_alpha, core_alpha
            )

    def _draw_ambient_bsm_edge(
        self,
        side: str,
        opacity: float,
        ...
        for index in range(steps):
            ...
            t = (t0 + t1) * 0.5
            if t <= 0.25:
                alpha = edge_alpha - (t / 0.25) * (edge_alpha - core_alpha)
            else:
                alpha = core_alpha * pow(max(0.0, 1.0 - (t - 0.25) / 0.75), 0.9)
            alpha *= opacity
            if alpha <= 0.0:
                continue
    ...
    def _draw_drive_status(self, state: ClusterUiState) -> None:
        ...
        gear_color = gear_status_color_for_theme(gear_text, theme)
    ...
    def _draw_follow_gap_status(self, state: ClusterUiState, bottom_y: float) -> None:
        ...
        bar_x = x + FOLLOW_STATUS_W - bars_total_w
